...
/**
 * 
 * Core indexer class that takes a web archive record and generates a Solr
 * record.
 * 
 * TODO Currently a rather crude, monolithic code structure. Should pull the
 * different metadata generation logic out into separate classes or at least
 * methods.
 * 
...
public class WARCIndexer {
	...
	/**
	 * Preferred constructor, allows passing in configuration from execution
	 * environment.
	 */
	public WARCIndexer(Config conf) throws NoSuchAlgorithmException {
		...
		if (conf.hasPath("warc.index.extract.content.annotations.enabled")
			...
			try {
				...
				throw new RuntimeException("Annotations failed with IOException when loading files " + annotationsFile
						+ ", " + openAccessSurtsFile);
			}
		}

		// We want stats for the 20 resource types that we spend the most time
		// processing
	...
	/**
	 * This extracts metadata and text from the ArchiveRecord and creates a suitable
	 * SolrRecord.
	 * 
	 * @param archiveName
	 * @param record
	...
	public SolrRecord extract(String archiveName, ArchiveRecord record) throws IOException {
		return this.extract(archiveName, record, this.extractText, null);
	}

	private String commaSepString(String[] values) {
...
//			nameBuilder.append("\"").append(n).append("\",");
	...
	/**
	 * This extracts metadata from the ArchiveRecord and creates a suitable
	 * SolrRecord. Removes the text field if flag set.
	 * 
	 * @param archiveName
	 * @param record
	 ...
	 * @throws IOException
	 */
	public SolrRecord extract(String archiveName, ArchiveRecord record, boolean isTextIncluded,
			TeamhCategories teamhcategory) throws IOException {
		final long start = System.nanoTime();
		ArchiveRecordHeader header = record.getHeader();
//        log.info("record extract " + record);
		...
		if (!header.getHeaderFields().isEmpty()) {
			if (header.getHeaderFieldKeys().contains(HEADER_KEY_TYPE)) {
				...
				// Bhaskar
				addSolrValues(solr, SolrFields.TEAMH_CATEGORIES, teamhcategory.getTeamhMainCategories());
				addSolrValues(solr, SolrFields.TEAMH_AGEGROUP, teamhcategory.getTeamhAgeGroup());
				addSolrValues(solr, SolrFields.TEAMH_SUBCATEGORIES, teamhcategory.getTeamhSubCategories());
			...
			log.debug("Current heap usage: " + FileUtils.byteCountToDisplaySize(Runtime.getRuntime().totalMemory()));
			...
			byte[] url_md5digest = md5.digest(Normalisation.sanitiseWARCHeaderValue(header.getUrl()).getBytes("UTF-8"));
			// String url_base64 =
			...
			// Get the length, but beware, this value also includes the HTTP headers (i.e.
			// it is the payload_length):
			...
			// If this is a revisit record, we should just return an update to the
			// crawl_dates (when using hashUrlId)
	...
	private void addSolrValues(SolrRecord solr, String solfrfield, String[] teamhMainCategories) {
//		solr.addField(solfrfield, "dummy");
		if (teamhMainCategories != null) {
			for (String category : teamhMainCategories) {
				solr.addField(solfrfield, category);
			}
		}
	}
	...
	protected URI parseURL(SolrRecord solr, String fullUrl) throws URIException {
		...
		org.apache.commons.httpclient.URI tempUri = new org.apache.commons.httpclient.URI(url.getEscapedURI(), false);
